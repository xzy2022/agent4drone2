"""
UAV Control Interface - Main GUI Application
Refactored to use the new modular architecture in src/
"""
import json
import os
import threading
...
class UAVAgentGUI:
    ...
    to UAVControlAgent class from the new modular architecture in src/.
    ...
    def _initialize_agent_worker(self, show_warnings: bool) -> None:
        """Worker thread to initialize the agent - delegates to UAVControlAgent from src.agents."""
        with self.agent_lock:
            ...
            # Delegate to UAVControlAgent from src.agents - it handles all LLM initialization logic
            try:
    ...
    def _fetch_session_summary(self, silent: bool) -> None:
        """Fetch session summary - delegates to UAVControlAgent method from src.agents."""
        with self.agent_lock:
            if not self.agent:
                return
    ...
    def _execute_command(self, command: str) -> None:
        """Execute command - delegates to UAVControlAgent.execute() method from src.agents."""
        with self.agent_lock:
            if not self.agent:
